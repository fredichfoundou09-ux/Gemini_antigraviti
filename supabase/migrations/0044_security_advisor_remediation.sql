...
--    -> Optimisation InitPlan (select auth.uid()) pour des performances optimales à l'échelle
--
...
--
-- 4. Optimisation des performances et déduplication des index
-- ==============================================================================
...
-- 1.4 Nettoyage des anciennes policies
...
-- 1.5 Politiques RLS pour learner_modules (Optimisées InitPlan)
...
CREATE POLICY "learner_modules_student_select" ON public.learner_modules
  ...
  USING (
    ...
    OR learner_id = (SELECT auth.uid())::text
  );
...
CREATE POLICY "learner_modules_teacher_select" ON public.learner_modules
  ...
  USING (
    EXISTS (
      ...
      WHERE t.user_id = (SELECT auth.uid()) AND tm.module_id::text = learner_modules.module_id
    )
    OR EXISTS (
      SELECT 1 FROM public.teachers t
      JOIN public.trainer_modules trm ON trm.trainer_id = t.id
      WHERE t.user_id = (SELECT auth.uid()) AND trm.module_id = learner_modules.module_id
    )
  );

-- 1.6 Politiques RLS pour trainer_modules (Optimisées InitPlan)
...
CREATE POLICY "trainer_modules_teacher_select" ON public.trainer_modules
  ...
  USING (
    ...
    OR trainer_id = (SELECT auth.uid())::text
  );
...
CREATE POLICY "trainer_modules_student_select" ON public.trainer_modules
  ...
  USING (
    EXISTS (
      ...
      WHERE s.user_id = (SELECT auth.uid()) AND sm.module_id::text = trainer_modules.module_id
    )
    OR EXISTS (
      SELECT 1 FROM public.students s
      JOIN public.learner_modules lm ON (lm.learner_id = s.id OR lm.learner_id = s.user_id::text)
      WHERE s.user_id = (SELECT auth.uid()) AND lm.module_id = trainer_modules.module_id
    )
  );
...
-- 2.1 Nettoyage des policies de la migration 0040 et précédentes
...
DROP POLICY IF EXISTS "course_files_staff_all" ON public.course_files;
DROP POLICY IF EXISTS "course_files_teacher_manage" ON public.course_files;
...
CREATE POLICY "courses_teacher_manage" ON public.courses
  ...
  USING (
    EXISTS (
      SELECT 1 FROM public.teachers t
      WHERE t.id = courses.teacher_id AND t.user_id = (SELECT auth.uid())
    )
  )
  WITH CHECK (
    EXISTS (
  ...
  USING (
    publie = true AND (
      EXISTS (
        SELECT 1 FROM public.students s
        JOIN public.student_modules sm ON sm.student_id = s.id
        WHERE s.user_id = (SELECT auth.uid()) AND sm.module_id = courses.module_id
      )
      OR EXISTS (
        SELECT 1 FROM public.students s
        JOIN public.learner_modules lm ON (lm.learner_id = s.id OR lm.learner_id = s.user_id::text)
        WHERE s.user_id = (SELECT auth.uid()) AND lm.module_id::text = courses.module_id::text
      )
      OR EXISTS (
...
CREATE POLICY "course_files_teacher_manage" ON public.course_files
  ...
  USING (
    EXISTS (
      ...
      WHERE c.id = course_files.course_id AND t.user_id = (SELECT auth.uid())
    )
  )
  WITH CHECK (
    EXISTS (
...
DROP POLICY IF EXISTS "submission_files_restricted_read" ON storage.objects;
...
CREATE POLICY "submission_files_student_insert" ON storage.objects
  ...
  WITH CHECK (
    bucket_id = 'submission-files' AND (
      ...
      OR EXISTS (
        SELECT 1 FROM public.students s
        WHERE s.user_id = (SELECT auth.uid()) AND (storage.foldername(name))[1] = s.id
      )
      OR public.is_staff()
    )
  );
  ...
  USING (
    bucket_id = 'submission-files' AND (
      ...
      OR EXISTS (
        ...
        WHERE asf.storage_path = storage.objects.name AND t.user_id = (SELECT auth.uid())
      )
    )
  );
...
-- ==============================================================================
-- SECTION 4 : INDEXATION PERFORMANCES ET DÉDUPLICATION
-- ==============================================================================

-- Suppression des doublons d'index signalés par l'Advisor
...
DROP INDEX IF EXISTS public.idx_courses_module_id;
DROP INDEX IF EXISTS public.idx_test_results_student_id;
DROP INDEX IF EXISTS public.idx_notifications_user_id;

-- Index optimisés uniques
